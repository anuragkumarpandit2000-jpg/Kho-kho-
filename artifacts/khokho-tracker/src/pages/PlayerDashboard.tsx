...
import { Activity, Trophy, Zap, ClipboardList, TrendingUp, Star } from "lucide-react";
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      {stats.improving && (
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Your scores are trending up over the last few sessions. Keep it going!</p>
          </div>
        </motion.div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-6 shadow-sm">
          <h3 className="font-black text-foreground mb-4 flex items-center gap-2">
            <Activity className="w-5 h-5 text-primary" /> Recent Entries
          </h3>
          {entries.length === 0 ? (
            ...
            <div className="space-y-2">
              {entries.slice(0, 5).map((entry) => (
                <div key={entry.date} className="flex items-center justify-between bg-secondary/30 rounded-xl px-4 py-2.5">
                  <span className="text-sm font-semibold text-foreground">{entry.date}</span>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-muted-foreground">Run: {entry.running}m</span>
